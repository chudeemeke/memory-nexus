...
export class SqliteEntityRepository implements IEntityRepository {
  ...
  async findById(id: number): Promise<Entity | null> {
    using statement = this.db.prepare<EntityRow, [number]>(
  ...
  async findByName(
    ...
    using statement = this.db.prepare<EntityRow, [string, string]>(
      `SELECT id, type, name, metadata, confidence, created_at
       FROM entities
       WHERE type = ? AND LOWER(name) = LOWER(?)`
    );
    const row = statement.get(type, normalizedName);
    if (!row) {
      return null;
    }
    return this.rowToEntity(row);
  ...
  async findBySession(sessionId: string): Promise<Entity[]> {
    using statement = this.db.prepare<EntityRow, [string]>(
  ...
  async save(entity: Entity): Promise<Entity> {
    ...
    using updateConfidenceStatement = this.prepareUpdateConfidence();
    // Check if entity already exists
    const existing = existsStatement.get(entity.type, entity.name);
    ...
    if (existing) {
      ...
      if (newConfidence > existing.confidence) {
        const updated = updateConfidenceStatement.run(newConfidence, existing.id);
        if (updated.changes === 0) throw new Error("Entity confidence update was not applied");
      }
    ...
    if (inserted.changes === 0) throw new Error("Entity insert was not applied");
    return entity.withId(Number(inserted.lastInsertRowid));
  }
    ...
    const saveAll = this.db.transaction((items: Entity[]) => {
      for (const entity of items) {
        ...
        if (existing) {
          ...
          if (inserted.changes === 0) throw new Error("Entity insert was not applied");
          results.push(entity.withId(Number(inserted.lastInsertRowid)));
        }
      }
    });
  ...
  private prepareExists() {
    return this.db.prepare<{ id: number; confidence: number } | null, [string, string]>(
      "SELECT id, confidence FROM entities WHERE type = ? AND LOWER(name) = LOWER(?)"
    );
  }

  private prepareUpdateConfidence() {
    return this.db.prepare(
