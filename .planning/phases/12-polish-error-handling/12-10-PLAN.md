...
must_haves:
  truths:
    - "All CLI commands respond to --help and exit successfully"
    - "Test suite catches 80%+ of intentionally injected bugs in domain layer"
    - "Running 'bun run mutation:domain' completes without failure"
  artifacts:
  ...
  <action>
    ...
    1. Create tests/smoke/cli-commands.test.ts:
       ...
       - Test suite: "CLI Smoke Tests"
         ...
         - Test: "memory completion --help exits 0"
...
3. All 17+ CLI commands have smoke tests (including completion)
