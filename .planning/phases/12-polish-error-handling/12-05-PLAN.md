...
must_haves:
  truths:
    ...
    - "Export then import produces identical data (round-trip)"
  artifacts:
...
<task type="auto">
  <name>Task 1: Create export/import service with round-trip verification</name>
  <files>
    src/application/services/export-service.ts
    src/application/services/export-service.test.ts
  ...
  <action>
    ...
    2. Create tests in export-service.test.ts:
       ...
       - **CRITICAL: Test round-trip integrity**:
         - Seed database with known data (sessions, messages, tool uses, entities, links)
         - Export to JSON file
         - Clear database
         - Import from exported file
         - Query all tables and verify data matches original seed data exactly
         - Compare counts AND content (not just counts)
...
4. Verify round-trip: export then import produces identical data (covered by Task 1 test)
</verification>

<success_criteria>
1. Export creates complete JSON backup with version field
2. Import restores data correctly
3. Round-trip (export -> import) preserves all data exactly
4. Validation catches invalid/corrupted files
5. --clear option works for fresh import
6. Stats displayed after both operations
7. Error handling with appropriate exit codes
</success_criteria>
