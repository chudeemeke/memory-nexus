...
> **RELEASE PROVENANCE CORRECTED 2026-07-06** - The `v4.0.2` ambiguity is resolved additively, not by rewriting tags. `v4.0.2` remains on `44a8707609bbc9246abc029a13ef2b16134c7786`; later commit `6155b683d4d846db79a52dbfd15aaebe9dfbc558` is documented as a post-release source fix/verification state, not the canonical release tag target. `v4.0.3` is annotated on release-source commit `2bc83131f3275c245b759edca666ae0975e7d86e`, with post-publish verification retained separately at `61514423398909ab0ddf53d4d024ff1b8e5b7347`. See `docs/releases/2026-07-06-release-provenance-v4.0.2-v4.0.3.md`.
...
> **ACTIVE EXECUTION CONTRACT 2026-09-19:** Resume from `.planning/memory-resilience/EXECUTION.md`, `work-items.json`, `JOURNAL.md`, and the native persistent goal. B01-B03 have scoped behavior proof. B04 is active: discovery/structural admission is implemented for 446 candidates/two packages, with 446 proposed classifications and zero unclassified; structural inventory validation now passes, without approving risk/applicability. No exclusions are approved. Source-bound package-output mapping and owned cleanup/evaluation/hook findings are recorded. All 446 files now have a four-metric candidate matrix. Static/runtime probes found selection and counter gaps owned by B05.1-B05.3. Next: independent risk/applicability review; ready fixture repairs can proceed while review is unavailable. B05 remains pending. The ledger has 218 items: 123 delivery/gates and 95 quality candidates. Complete four-metric, changed-line, hosted CI and independent-review gates remain open. Routine authorized work proceeds autonomously; R03/P03/A03 retain owner decisions and PR #1 remains draft.
...
### Current Resume Point - 2026-07-05

**Completed this review/update pass:**

- Completed Phase 43 Market-Leader and Sales-Readiness Gate for the scoped local-first CLI/API release lane.
- Promoted the remaining `remote_sync_conflict` market blocker to behavior-backed eval coverage; `bun run eval:v5:market` now passes 9/9 with 0 blockers.
- Refreshed `CLAUDE.md` so future agents see `memory` / `@chude/memory` as the first-party infrastructure tool, Phase 43 as complete, and Phase 44 as the active release-candidate handoff.
- Fixed a Windows hook-launcher defect in source: background sync resolves and launches `memory sync` directly instead of assuming `aidev memory sync` through a shell wrapper.
- Verified final Phase 43 gates: typecheck, build, full tests (4,455 pass, 0 fail), test isolation, eval:v5, eval:v5:market, coverage statements 97.31%, branches 95.00%, functions 96.51%, lines 97.39%, dependency audit, gitleaks, diff whitespace, npm pack dry-run, and package privacy check.
...
- At that 2026-06-07 point, `eval:v5` passed 9/9 fixtures with 8 behavior-backed fixtures and 1 contract fixture, and `eval:v5:market` intentionally failed until Phase 43 promoted or dispositioned `remote_sync_conflict`.
...
- Historical next step at 2026-06-07 was Phase 43. Current 2026-07-05 truth supersedes this: Phase 43 is complete and Phase 44 is next.
